...
class User():
    ...
    def password(self, pwd):
        ...
        """
        if pwd is None or not isinstance(pwd, str):  # Changed type check to isinstance
            self.__password = None
        else:
            self.__password = hashlib.md5(pwd.encode()).hexdigest().lower()  # Corrected variable name
    ...
    def is_valid_password(self, pwd):
        ...
        """
        if self.__password is None:  # Removed redundant checks for None and non-string type
            return False
        return hashlib.md5(pwd.encode()).hexdigest().lower() == self.__password  # Use lower() for case-insensitive comparison
